...
if ismember(path(end),["/" "\"])
    error(AIDIF.Constants.ERROR_ID_INVALID_PATH_FORMAT , 'rootPath must not end with a file separator.');
end
...
if ~all(separatorCount == separatorCount(1))
    error(AIDIF.Constants.ERROR_ID_INCONSISTENT_STRUCTURE, 'The file path has an inconsistent hive structure.');
end
end
