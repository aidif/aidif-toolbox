...
function validateInputTable(tt)
    if ~ismember('basal_rate', tt.Properties.VariableNames)
        error(TestHelpers.ERROR_ID_INVALID_ARGUMENT, "''tt'' must have a ''basal_rate'' column.");
    end
    br = tt.basal_rate;
    ...
    if height(tt) < 2
        error(TestHelpers.ERROR_ID_INVALID_ARGUMENT, "''tt'' must contain at least two samples to be resampled.");
    end
    if ~issorted(tt.Properties.RowTimes,"ascend")
        error(TestHelpers.ERROR_ID_INVALID_ARGUMENT, "''tt''must be sorted ascending by time.");
    end
end
